...
const getFilteredTrips = async (req, res) => {
    try {
        ...
        //todo remove any
        const trips = await Trip_1.default.find(params);
        let formedTrips = [];
        for (const trip of trips) {
            const driver = await userController_1.default.getDriver(trip.driverID);
...
const putApprovePassenger = async (req, res) => {
    try {
        ...
        await Trip_1.default.updateOne({ _id: tripId, "passengerIDs.bookingId": bookingId }, { $set: { "passengerIDs.$.status": "Approved" } }, { new: true });
        const creditsInQuestion = Number(totalCredits);
        const passenger = await User_1.default.findOne({ userId: passengerId });
        passenger.expoPushToken && (0, userUtils_1.sendPushNotification)(passenger.expoPushToken, 'Booking request accepted', 'Have a safe journey! 🚙');
        const currentPassengerOnHold = Number(passenger.credits.onHold);
        await User_1.default.updateOne({ userId: passenger.userId }, { $set: { 'credits.onHold': currentPassengerOnHold - creditsInQuestion } });
        const currentDriverEarningOnHold = Number(validatedUser.user.credits.earningsOnHold);
        await User_1.default.updateOne({ userId: validatedUser.userId }, { $set: { 'credits.earningsOnHold': currentDriverEarningOnHold + creditsInQuestion } });
        const updatedTrip = await Trip_1.default.findOne({ _id: tripId });
...
const putRejectPassenger = async (req, res) => {
    try {
        ...
        await Trip_1.default.updateOne({ _id: tripId, "passengerIDs.bookingId": bookingId }, {
...
const putMakeRequest = async (req, res) => {
    try {
        ...
        await User_1.default.updateOne({ userId: validatedUser.user.userId }, {
        ...
        const driver = await User_1.default.findOne({ userId: trip.driverID });
        driver.expoPushToken && (0, userUtils_1.sendPushNotification)(driver.expoPushToken, 'Incoming booking request', 'Confirm it now!');
        const updatedTrip = await Trip_1.default.findOne({ _id: tripId });
        return res.status(200).json({ message: 'Trip edited successfully', trip: updatedTrip });
    }
    catch (error) {
